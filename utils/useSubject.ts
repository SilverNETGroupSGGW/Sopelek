...
export default function useSubject() {
  ...
  function calculatePosition(subject: Subject, groups: string[]) {
    ...
    const y = groups.findIndex(group => subject.groups!.some(subjectGroup => subjectGroup.name === group)) * runtimeConfig.public.cellHeight

    const height = subject.groups!.length * runtimeConfig.public.cellHeight
  ...
  function calculateGroups(subject: Subject) {
    const currentGroupIndex = subject.y! / runtimeConfig.public.cellHeight
    const newGroupCount = subject.height! / runtimeConfig.public.cellHeight
