...
import { SelectContent, SelectGroup, SelectItem, SelectItemText, SelectLabel, SelectPortal, SelectRoot, SelectTrigger, SelectValue, SelectViewport } from 'radix-vue'
...
const computedModel = computed(() => props.options.find(option => option.value === model.value)?.label,
)
</script>
    ...
    <SelectRoot v-model="model">
      ...
      <SelectPortal>
        <SelectContent class="absolute z-10 mt-4 max-h-[var(--radix-select-content-available-height)] w-[var(--radix-select-trigger-width)] overflow-auto rounded-lg border border-gray-200 bg-white px-1 py-2 text-base shadow-lg focus:outline-none" position="popper" :side-offset="2">
          <SelectViewport>
            <SelectGroup>
              <SelectItem v-for="(option, index) in options" :key="index" :value="option.value" class="relative mt-1 cursor-pointer select-none rounded-lg bg-white px-5 py-2 text-gray-900 data-[highlighted]:bg-indigo-600 data-[highlighted]:text-white">
